...
class RingSegmentation:
    ...
    def find_rings(self):
        ...
        for c in candidates:
            ...
            self.soundhandle.say(res.color, self.voice, self.volume)
    ...
    def add_ring(self, pose: Pose, res):
        ...
        for e in self.rings:
            if (
                ...
                req = ColorClassifierServiceRequest()
                req.mode = 1
                req.color = ColorRGBA(res.marker_color.r, res.marker_color.g, res.marker_color.b, 1)
                res = self.get_color(req)
                if (not (res.color == "black" and e.color_name != "black")):
                    e.color_name = res.color
