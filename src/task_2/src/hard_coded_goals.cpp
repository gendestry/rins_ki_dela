...
#include <unistd.h>
#include <stdlib.h>
#include "task_2/PoseAndColorArray.h"
...
std::vector<task_2::PoseAndColor> cylinders;
std::vector<task_2::PoseAndColor> rings;
...
void areWeDone() {
    if(cylinders.size() >= 3 && rings.size() > 3 && isGreen) {
        ROS_INFO("We are done!");

        for (auto ring : rings) {
            if (ring.color == "green") {
                ROS_INFO("Go winning - x: %f y: %f", ring.pose.position.x, ring.pose.position.y);
                moveToSimple(ring.pose.position.x + 0.25, ring.pose.position.y);
                break;
            }
        }
    }
...
void cylinderCallback(const task_2::PoseAndColorArray goal) {
    std::stringstream ss;
    ss << "Cylinders:";

    cylinders.clear();
    for(auto cyl : goal.cylinders) {
        ss << " " << cyl.color;
    ...
    if(cylinders.size() >= 3)
        ROS_INFO("Three cylinders found!");

    areWeDone();
}

void ringCallback(const task_2::PoseAndColorArray goal) {
    // ROS_INFO("Ring callback");
    std::stringstream ss;
    ss << "Rings:";

    rings.clear();
    for(auto r : goal.cylinders) {
        // ROS_INFO("%s ", r.color.c_str());
        rings.push_back(r);
        ss << " " << r.color;
...
int main(int argc, char** argv) {
    ...
    ring_sub = n.subscribe("ring_pose", 10, &ringCallback);
    cylinder_sub = n.subscribe("cylinder_pose", 10, &cylinderCallback);
