...
class Arm_Mover():
    def __init__(self):
        ...
        self.back = JointTrajectory()
        self.back.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint", "arm_wrist_flex_joint"]
        self.back.points = [JointTrajectoryPoint(positions=[-3.3,0.3,1,0],
                                                    time_from_start = rospy.Duration(1))]
    ...
    def update_position(self):
        ...
        if self.send_command:
            if self.user_command == 'retract':
                ...
                print('Left-ed arm!')
            elif self.user_command == 'back':
                self.arm_movement_pub.publish(self.back)
                print('Bucket arm!')
            else:
